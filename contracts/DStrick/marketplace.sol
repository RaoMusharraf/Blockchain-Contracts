...
contract Marketplace is ReentrancyGuard , Ownable{
    ...
    constructor(address ERC721NFT, address admin){
    ...
    */
    function BuyAdmin (uint256 price,uint256 tokenId) public payable{
        tokenID.increment();
        IIERC721(MinterAddress).safeMint(msg.sender,tokenId);
        payable(AddminAddress).transfer(price);
        Id[tokenId] = tokenID.current();
        // IERC20(paymentToken).safeTransferFrom(msg.sender, AddminAddress , price);
        _idToNFT[tokenID.current()] = NFT(tokenId,msg.sender,msg.sender,price,true);
    }
    // ============ ListNft FUNCTIONS ============
    /*
    ...
    */
    function buyNft(uint256 _tokenId) public payable nonReentrant {
        ...
        require(msg.value >= _idToNFT[Id[_tokenId]].price , "Not enough ether to cover asking price !!!");
